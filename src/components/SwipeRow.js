...
import { applySimpleTension } from "../util/gesture/index";
import { getWidth, getHeight } from "../util/layout/index";
import {
...
const SwipeRow = createReactClass({
  ...
  runOpenAndClose(currentPosition, endPosition, speed) {
    requestAnimationFrame((timestamp) => {
      ...
      if (absCurrent >= absEnd + 75) {
        return this.onSwipeEnd(null, { dx: 0, vx: 1 }).then(() =>
          this.resolveCallout(true)
        );
      }
  ...
  checkHandleSubViewWidthUpdate(nextState) {
    let updateLeft =
      this.state.leftSubViewWidth !== nextState.leftSubViewWidth &&
      nextState.leftSubViewWidth >= 0;
    let updateRight =
      this.state.rightSubViewWidth !== nextState.rightSubViewWidth &&
      nextState.rightSubViewWidth >= 0;
    if (
      nextState.open &&
      !nextState.heightAnimating &&
      (updateLeft || updateRight)
    ) {
      let activeSide = nextState.activeSide;
      let position =
        activeSide === "left"
          ? nextState.leftSubViewWidth
          : nextState.rightSubViewWidth;
  ...
  onSwipeEnd(e, g) {
    ...
    if (this.state.open) {
      ...
      (this.state.activeSide === "left" && dx > 0) ||
      (this.state.activeSide === "right" && dx < 0)
    ) {
      return this.checkAnimateOpenOrClose(dx, vx);
    } else {
  ...
  updatePanPosition(dx) {
    ...
    if (this.state.activeSide === "left") {
      ...
      dx =
        relativeDX >= threshold
          ? dx
          : -(this.state.leftSubViewWidth - threshold);
      if (dx > 0) {
      ...
      dx =
        relativeDX >= threshold ? dx : this.state.rightSubViewWidth - threshold;
      if (dx < 0) {
        dx = this.applyTensionToGesture(dx);
      }
      this.setPanPosition(dx);
  ...
  isPastOpenThreshold(dx, vx) {
    if (
      (dx > 0 && this.props.leftSubView) ||
      (dx < 0 && this.props.rightSubView)
    ) {
      let thresholdBase =
        this.getActiveSubViewWidth() * OPEN_POSITION_THRESHOLD_FACTOR;
  ...
  isPastCloseThreshold(dx, vx) {
    let thresholdBase =
      this.getActiveSubViewWidth() * CLOSE_POSITION_THRESHOLD_FACTOR;
    let absVX = Math.abs(vx);
    let velocityMod = absVX * (thresholdBase * 2);
    let threshold = thresholdBase - velocityMod;
  ...
  shouldCloseOnClick() {
    let options =
      this.state.activeSide === "left"
        ? this.props.leftSubViewOptions
        : this.props.rightSubViewOptions;
    let closeOnPress = options.fullWidth ? false : options.closeOnPress;
    return isDefined(closeOnPress)
      ? closeOnPress
      : defaultSubViewOptions.closeOnPress;
  },

  render() {
    return (
      <Animated.View
        style={[styles.container, this.props.style, this.getHeightStyle()]}
  ...
  getRowPointerEvents() {
    return this.state.open && this.props.blockChildEventsWhenOpen
      ? "none"
      : "box-none";
  },
  ...
  renderSubView() {
    ...
    if (activeSide && subView) {
      return (
        <Animated.View
          style={[styles.subViewContainer, activeSideStyle, panStyle]}
          {...this.checkGetBlockCloseTimeout(isLeft)}
        >
          <View
            style={[styles.subViewWrapper, wrapperStyle]}
            onLayout={onLayout}
          >
  ...
  getSubViewWrapperStyle(isLeft) {
    ...
    fullWidth = isDefined(fullWidth)
      ? fullWidth
      : defaultSubViewOptions.fullWidth;
    return {
