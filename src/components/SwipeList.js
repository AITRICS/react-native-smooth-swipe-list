...
const SwipeList = createReactClass({
    ...
    if (this.props.rowData && nextRowData.length < this.props.rowData.length) {
      ...
      if (rowRefs.length) {
        rowRefs.forEach((ref) => {
          ...
          component &&
            component.animateOut(() => this.updateDataSource(nextRowData));
        });
      }
    } else {
  ...
  checkAnimateAddRow(nextRowData = []) {
    let rowsAdded = this.props.rowData
      ? nextRowData.length - this.props.rowData.length
      : nextRowData.length;
    if (rowsAdded > 0) {
